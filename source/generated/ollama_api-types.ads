...
package Ollama_API.Types is
   ...
   type Float_Vector is tagged private
   with
     Variable_Indexing => Get_Float_Variable_Reference,
     Constant_Indexing => Get_Float_Constant_Reference,
     Aggregate         => (Empty => Empty, Add_Unnamed => Append);
   ...
   type ChatResponse is record
      ...
      total_duration       : Optional_Integer;
      --  Total time spent generating in nanoseconds
      load_duration        : Optional_Integer;
      --  Time spent loading the model in nanoseconds
      prompt_eval_count    : Optional_Integer;
      --  Number of tokens in the prompt
      prompt_eval_duration : Optional_Integer;
      --  Time spent evaluating the prompt in nanoseconds
      eval_count           : Optional_Integer;
      --  Number of tokens generated in the response
      eval_duration        : Optional_Integer;
   ...
   type StatusEvent is record
      ...
      total     : Optional_Integer;
      --  Total number of bytes expected for the operation
      completed : Optional_Integer;
   ...
   type TokenLogprob is record
      ...
      logprob : Optional_Float;
      --  The log probability of this token
      bytes   : Integer_Vector;
   ...
   type Ps is record
      ...
      size_vram      : Optional_Integer;
      --  VRAM usage in bytes
      context_length : Optional_Integer;
   ...
   function Empty return Integer_Vector;
   ...
   function Empty return Float_Vector;

   function Is_Null (Self : Float_Vector) return Boolean;

   function Length (Self : Float_Vector) return Natural;

   procedure Clear (Self : in out Float_Vector; Is_Null : Boolean := True);

   procedure Append (Self : in out Float_Vector; Value : Float);
   ...
   type Float_Variable_Reference (Element : not null access Float) is
   null record
   with Implicit_Dereference => Element;

   not overriding
   function Get_Float_Variable_Reference
     (Self : aliased in out Float_Vector; Index : Positive)
      return Float_Variable_Reference
   with Inline;

   type Float_Constant_Reference
     (Element : not null access constant Float)
   is null record
   with Implicit_Dereference => Element;

   not overriding
   function Get_Float_Constant_Reference
     (Self : aliased Float_Vector; Index : Positive)
      return Float_Constant_Reference
   with Inline;
...
private
   ...
   type Float_Vector is new Ada.Finalization.Controlled with record
      Data    : Float_Array_Access;
      Length  : Natural := 0;
      Is_Null : Boolean := True;
   end record;

   overriding
   procedure Adjust (Self : in out Float_Vector);

   overriding
   procedure Finalize (Self : in out Float_Vector);
