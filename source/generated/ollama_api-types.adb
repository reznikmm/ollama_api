...
package body Ollama_API.Types is
   ...
   begin
      if Self.Length = 0 then
         ...
         Self.Data := new Logprob_Array (1 .. 3 * Self.Length / 2);
   ...
   begin
      if Self.Length = 0 then
         ...
         Self.Data := new TokenLogprob_Array (1 .. 3 * Self.Length / 2);
         Self.Data (1 .. Self.Length) := Self_Data_Saved.all;
         Free (Self_Data_Saved);
      end if;
      Self.Length := Self.Length + 1;
   ...
   procedure Free is new
     Ada.Unchecked_Deallocation (Integer_Array, Integer_Array_Access);

   overriding
   procedure Adjust (Self : in out Integer_Vector) is
   begin
      if Self.Length > 0 then
   ...
   overriding
   procedure Finalize (Self : in out Integer_Vector) is
   begin
      Free (Self.Data);
      Self.Length := 0;
   end Finalize;
   ...
   procedure Append (Self : in out Integer_Vector; Value : Integer) is
      Init_Length     : constant Positive :=
        Positive'Max (2, 256 / Integer'Size);
   ...
   procedure Free is new
     Ada.Unchecked_Deallocation (Float_Array, Float_Array_Access);

   overriding
   procedure Adjust (Self : in out Float_Vector) is
   begin
      if Self.Length > 0 then
         Self.Data := new Float_Array'(Self.Data (1 .. Self.Length));
      end if;
   end Adjust;

   overriding
   procedure Finalize (Self : in out Float_Vector) is
   begin
      Free (Self.Data);
      Self.Length := 0;
   end Finalize;
   ...
   procedure Clear (Self : in out Float_Vector; Is_Null : Boolean := True) is
   begin
      Self.Length := 0;
      Self.Is_Null := Is_Null;
   end Clear;

   procedure Append (Self : in out Float_Vector; Value : Float) is
      Init_Length     : constant Positive :=
        Positive'Max (2, 256 / Float'Size);
      Self_Data_Saved : Float_Array_Access := Self.Data;
   begin
      if Self.Length = 0 then
         Self.Is_Null := False;
         Self.Data := new Float_Array (1 .. Init_Length);
      elsif Self.Length = Self.Data'Last then
         Self.Data := new Float_Array (1 .. 3 * Self.Length / 2);
         Self.Data (1 .. Self.Length) := Self_Data_Saved.all;
         Free (Self_Data_Saved);
      end if;
      Self.Length := Self.Length + 1;
   ...
   begin
      if Self.Length = 0 then
         ...
         Self.Data := new ToolDefinition_Array (1 .. 3 * Self.Length / 2);
         Self.Data (1 .. Self.Length) := Self_Data_Saved.all;
         Free (Self_Data_Saved);
      end if;
      Self.Length := Self.Length + 1;
   ...
   begin
      if Self.Length = 0 then
         ...
         Self.Data := new WebSearchResult_Array (1 .. 3 * Self.Length / 2);
         Self.Data (1 .. Self.Length) := Self_Data_Saved.all;
         Free (Self_Data_Saved);
      end if;
      Self.Length := Self.Length + 1;
   ...
   begin
      if Self.Length = 0 then
         ...
         Self.Data := new Ps_Array (1 .. 3 * Self.Length / 2);
         Self.Data (1 .. Self.Length) := Self_Data_Saved.all;
         Free (Self_Data_Saved);
      end if;
      Self.Length := Self.Length + 1;
   ...
   begin
      if Self.Length = 0 then
         ...
         Self.Data := new ChatMessage_Array (1 .. 3 * Self.Length / 2);
         Self.Data (1 .. Self.Length) := Self_Data_Saved.all;
         Free (Self_Data_Saved);
      end if;
      Self.Length := Self.Length + 1;
   ...
   begin
      if Self.Length = 0 then
         ...
         Self.Data := new ToolCall_Array (1 .. 3 * Self.Length / 2);
         Self.Data (1 .. Self.Length) := Self_Data_Saved.all;
         Free (Self_Data_Saved);
      end if;
      Self.Length := Self.Length + 1;
   ...
   begin
      if Self.Length = 0 then
         ...
         Self.Data := new ModelSummary_Array (1 .. 3 * Self.Length / 2);
         Self.Data (1 .. Self.Length) := Self_Data_Saved.all;
         Free (Self_Data_Saved);
      end if;
      Self.Length := Self.Length + 1;
